...
def build_weekly_review_pack(
    ...
    *,
    ...
    weekly_pack["top_attention"] = [_build_attention_explainability(item) for item in top_attention]
    weekly_pack["repo_briefings"] = [_build_repo_briefing_explainability(item) for item in repo_briefings]
    weekly_pack["weekly_story_v1"] = _build_weekly_story_v1(weekly_pack)
    return weekly_pack
...
def _build_weekly_story_v1(weekly_pack: dict[str, Any]) -> dict[str, Any]:
    sections = [
        ...
        },
        {
            ...
            "evidence_items": (
                _campaign_evidence_items(list(weekly_pack.get("top_drift_returned_campaigns") or []), summary_key="summary", kind="post-apply-monitoring")
                + _campaign_evidence_items(list(weekly_pack.get("top_reopened_campaigns") or []), summary_key="summary", kind="post-apply-monitoring", label_prefix="Reopened")
                + _campaign_evidence_items(list(weekly_pack.get("top_monitor_now_campaigns") or []), summary_key="summary", kind="post-apply-monitoring", label_prefix="Monitor Now")
                + _campaign_evidence_items(list(weekly_pack.get("top_holding_clean_campaigns") or []), summary_key="summary", kind="post-apply-monitoring", label_prefix="Holding Clean")
            ),
        },
        {
            "id": "campaign-tuning",
            ...
            "next_step": str(weekly_pack.get("next_tie_break_candidate") or weekly_pack.get("next_tuned_campaign") or NO_NEXT_TUNED_CAMPAIGN),
            "next_label": ACTION_SYNC_CANONICAL_LABELS["next_tie_break_candidate"],
            "reason_codes": ["action-sync", "campaign-tuning"],
            "evidence_items": (
                _campaign_evidence_items(list(weekly_pack.get("top_proven_campaigns") or []), summary_key="summary", kind="campaign-tuning", label_prefix="Proven")
                + _campaign_evidence_items(list(weekly_pack.get("top_caution_campaigns") or []), summary_key="summary", kind="campaign-tuning", label_prefix="Caution")
                + _campaign_evidence_items(list(weekly_pack.get("top_thin_evidence_campaigns") or []), summary_key="summary", kind="campaign-tuning", label_prefix="Thin Evidence")
            ),
        },
        {
            ...
            "evidence_items": (
                ...
                + _repo_evidence_items(list(weekly_pack.get("top_persistent_pressure_repos") or []), label_prefix="Persistent Pressure")
                + _repo_evidence_items(list(weekly_pack.get("top_improving_repos") or []), label_prefix="Improving After Intervention")
        ...
        },
        {
            ...
            "evidence_items": (
                _approval_evidence_items(list(weekly_pack.get("top_needs_reapproval_approvals") or []), label_prefix="Needs Re-Approval")
                + _approval_evidence_items(list(weekly_pack.get("top_ready_for_review_approvals") or []), label_prefix="Ready For Review")
                + _approval_evidence_items(list(weekly_pack.get("top_approved_manual_approvals") or []), label_prefix="Approved But Manual")
        ...
        },
        {
            ...
            "evidence_items": (
                [_build_story_evidence_item(str(item.get("repo") or item.get("title") or "Operator item"), str(item.get("operator_focus_summary") or NO_OPERATOR_FOCUS_SUMMARY), "operator-focus") for item in list(weekly_pack.get("top_act_now_items") or [])[:3]]
                + [_build_story_evidence_item(str(item.get("repo") or item.get("title") or "Operator item"), str(item.get("operator_focus_summary") or NO_OPERATOR_FOCUS_SUMMARY), "operator-focus") for item in list(weekly_pack.get("top_watch_closely_items") or [])[:3]]
                + [_build_story_evidence_item(str(item.get("repo") or item.get("title") or "Operator item"), str(item.get("operator_focus_summary") or NO_OPERATOR_FOCUS_SUMMARY), "operator-focus") for item in list(weekly_pack.get("top_improving_items") or [])[:3]]
                + [_build_story_evidence_item(str(item.get("repo") or item.get("title") or "Operator item"), str(item.get("operator_focus_summary") or NO_OPERATOR_FOCUS_SUMMARY), "operator-focus") for item in list(weekly_pack.get("top_fragile_items") or [])[:3]]
                + [_build_story_evidence_item(str(item.get("repo") or item.get("title") or "Operator item"), str(item.get("operator_focus_summary") or NO_OPERATOR_FOCUS_SUMMARY), "operator-focus") for item in list(weekly_pack.get("top_revalidate_items") or [])[:3]]
            ),
        },
    ]
    ...
    return {
        ...
        "next_step": str(weekly_pack.get("next_best_workflow_step") or "Open the standard workbook first, then use --control-center for read-only triage."),
