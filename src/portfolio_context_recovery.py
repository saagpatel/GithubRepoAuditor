...
def _build_context_sections(
    ...
    run_instructions = _correct_run_instructions(run_instructions, project_path)
...
_CI_PM_MARKERS = {
    "pnpm": ("pnpm install", "pnpm/action-setup", "pnpm test", "pnpm run"),
    "yarn": ("yarn install --frozen", "yarn install --immutable"),
    "bun": ("bun install",),
}


def _detect_package_manager(project_path: Path) -> str | None:
    """Detect canonical JS package manager from lockfile, then CI workflow, then default to npm.

    Returns None for non-JS projects.
    """
    for pm, lockfile in _PM_LOCKFILES.items():
        if (project_path / lockfile).is_file():
            return pm
    workflows_dir = project_path / ".github" / "workflows"
    if workflows_dir.is_dir():
        for ext in ("*.yml", "*.yaml"):
            for workflow in workflows_dir.glob(ext):
                try:
                    text = workflow.read_text(errors="ignore")
                except OSError:
                    continue
                for pm, markers in _CI_PM_MARKERS.items():
...
_PRESCRIPTIVE_CMD_PATTERN = re.compile(
    r"\b(npm|yarn|pnpm|bun)\s+(install|run|test|build|exec|ci|add)\b",
    re.IGNORECASE,
)
_USE_ONLY_PATTERN = re.compile(
    r"(?im)^.*\buse\s+[`'\"]?(?:npm|yarn|pnpm|bun)[`'\"]?\s+only.*$\n?",
)
_MANUAL_MARKER = "<!-- manual:run-instructions -->"


def _correct_run_instructions(text: str, project_path: Path) -> str:
    """Rewrite run-instruction text when its prescribed package manager contradicts detection.

    Pass-through when:
    - The operator-marker `<!-- manual:run-instructions -->` is present (intentional override).
    - No JS package manager is detected (non-JS project).
    - Detected manager is npm (default fallback, no contradiction possible).
    - No contradicting prescriptive language is found.

    Otherwise: rewrite `X install/run/test/...` commands to use the detected manager,
    drop "use X only" lines, and prepend a breadcrumb so operators can audit the correction.
    """
    if _MANUAL_MARKER in text:
        return text
    detected = _detect_package_manager(project_path)
    if not detected or detected == "npm":
        return text
    contradictions: set[str] = set()
    for match in _PRESCRIPTIVE_CMD_PATTERN.finditer(text):
        pm = match.group(1).lower()
        if pm != detected:
            contradictions.add(pm)
    if _USE_ONLY_PATTERN.search(text):
        contradictions.add("explicit")
    ...
    breadcrumb = (
        f"[corrected: detected {detected} (lockfile or CI workflow); "
        f"previous text mentioned a different package manager and was rewritten]\n\n"
    )
    return breadcrumb + corrected.lstrip()
