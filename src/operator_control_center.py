...
def build_operator_snapshot(
    ...
    *,
    ...
    summary = {
        ...
        "primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_score": resolution_trend["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_score"],
        "primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status": resolution_trend["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status"],
        "primary_target_closure_forecast_reset_reentry_rebuild_reentry_status": resolution_trend["primary_target_closure_forecast_reset_reentry_rebuild_reentry_status"],
        "primary_target_closure_forecast_reset_reentry_rebuild_reentry_reason": resolution_trend["primary_target_closure_forecast_reset_reentry_rebuild_reentry_reason"],
        "closure_forecast_reset_reentry_rebuild_refresh_recovery_summary": resolution_trend["closure_forecast_reset_reentry_rebuild_refresh_recovery_summary"],
        "closure_forecast_reset_reentry_rebuild_reentry_summary": resolution_trend["closure_forecast_reset_reentry_rebuild_reentry_summary"],
        "closure_forecast_reset_reentry_rebuild_refresh_window_runs": resolution_trend["closure_forecast_reset_reentry_rebuild_refresh_window_runs"],
        "recovering_from_confirmation_rebuild_reset_hotspots": resolution_trend["recovering_from_confirmation_rebuild_reset_hotspots"],
        "recovering_from_clearance_rebuild_reset_hotspots": resolution_trend["recovering_from_clearance_rebuild_reset_hotspots"],
...
def _build_resolution_trend(
    ...
    *,
    ...
    reset_reentry_rebuild_recovery = _apply_reset_reentry_rebuild_refresh_recovery_and_reentry(
        resolution_targets,
        history,
        current_generated_at=current_generated_at,
        confidence_calibration=confidence_calibration,
    ...
    return {
        ...
        "primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_score": primary_target.get("closure_forecast_reset_reentry_rebuild_refresh_recovery_score", reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_score"]) if primary_target else reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_score"],
        "primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status": primary_target.get("closure_forecast_reset_reentry_rebuild_refresh_recovery_status", reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status"]) if primary_target else reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status"],
        "primary_target_closure_forecast_reset_reentry_rebuild_reentry_status": primary_target.get("closure_forecast_reset_reentry_rebuild_reentry_status", reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_reentry_status"]) if primary_target else reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_reentry_status"],
        "primary_target_closure_forecast_reset_reentry_rebuild_reentry_reason": primary_target.get("closure_forecast_reset_reentry_rebuild_reentry_reason", reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_reentry_reason"]) if primary_target else reset_reentry_rebuild_recovery["primary_target_closure_forecast_reset_reentry_rebuild_reentry_reason"],
        "closure_forecast_reset_reentry_rebuild_refresh_recovery_summary": reset_reentry_rebuild_recovery["closure_forecast_reset_reentry_rebuild_refresh_recovery_summary"],
        "closure_forecast_reset_reentry_rebuild_reentry_summary": reset_reentry_rebuild_recovery["closure_forecast_reset_reentry_rebuild_reentry_summary"],
        "closure_forecast_reset_reentry_rebuild_refresh_window_runs": reset_reentry_rebuild_recovery["closure_forecast_reset_reentry_rebuild_refresh_window_runs"],
        "recovering_from_confirmation_rebuild_reset_hotspots": reset_reentry_rebuild_recovery["recovering_from_confirmation_rebuild_reset_hotspots"],
        "recovering_from_clearance_rebuild_reset_hotspots": reset_reentry_rebuild_recovery["recovering_from_clearance_rebuild_reset_hotspots"],
...
def _closure_forecast_reset_reentry_rebuild_refresh_recovery_for_target(
    ...
    if (
        ...
        reentry_reason = (
            "Fresh confirmation-side follow-through has re-earned stronger rebuilt confirmation posture."
        )
    elif (
        recovery_status == "reentering-clearance-rebuild"
        and current_freshness == "fresh"
        ...
        reentry_status = "blocked"
        reentry_reason = (
            "Local target instability is still preventing positive confirmation-side rebuilt re-entry."
        )
    elif recovery_status in {
        "recovering-confirmation-rebuild-reset",
        "reentering-confirmation-rebuild",
        ...
        reentry_reason = (
            "Fresh confirmation-side evidence is returning after rebuilt posture was softened or reset, but it has not yet re-earned stronger rebuilt posture."
        )
    elif recovery_status in {
        "recovering-clearance-rebuild-reset",
        "reentering-clearance-rebuild",
        ...
        reentry_reason = (
            "Fresh clearance-side evidence is returning after rebuilt posture was softened or reset, but it has not yet re-earned stronger rebuilt posture."
        )
    else:
        reentry_status = "none"
        reentry_reason = ""
...
def _closure_forecast_reset_reentry_rebuild_refresh_recovery_summary(
    ...
    if status == "recovering-confirmation-rebuild-reset":
        return (
            f"Fresh confirmation-side evidence is returning after rebuilt posture softened or reset for {label}, "
            f"but it has not yet re-earned stronger rebuilt posture ({score:.2f})."
        )
    if status == "recovering-clearance-rebuild-reset":
        return (
            f"Fresh clearance-side evidence is returning after rebuilt posture softened or reset for {label}, "
            f"but it has not yet re-earned stronger rebuilt posture ({score:.2f})."
        )
    if status == "reentering-confirmation-rebuild":
        return (
            f"Confirmation-side rebuilt posture for {label} is recovering strongly enough that stronger restored posture may be re-earned soon ({score:.2f})."
        )
    if status == "reentering-clearance-rebuild":
        return (
            f"Clearance-side rebuilt posture for {label} is recovering strongly enough that stronger restored caution may be re-earned soon ({score:.2f})."
        )
    if status == "reversing":
        return (
            f"The post-reset rebuilt recovery attempt for {label} is changing direction, so stronger rebuilt posture stays blocked ({score:.2f})."
        )
    if status == "blocked":
        return primary_target.get(
            "closure_forecast_reset_reentry_rebuild_reentry_reason",
            f"Local target instability is still preventing positive confirmation-side rebuilt re-entry for {label}.",
        )
    if recovering_confirmation_hotspots:
        hotspot = recovering_confirmation_hotspots[0]
        return (
            f"Confirmation-side rebuilt recovery is strongest around {hotspot.get('label', 'recent hotspots')}, "
            "so those classes are closest to re-earning stronger rebuilt confirmation posture."
        )
    if recovering_clearance_hotspots:
        hotspot = recovering_clearance_hotspots[0]
        return (
            f"Clearance-side rebuilt recovery is strongest around {hotspot.get('label', 'recent hotspots')}, "
            "so those classes are closest to re-earning stronger rebuilt clearance posture."
        )
    return "No rebuilt reset re-entry recovery attempt is active enough yet to re-earn stronger restored posture."
...
def _closure_forecast_reset_reentry_rebuild_reentry_summary(
    ...
    if status == "pending-confirmation-rebuild-reentry":
        return (
            f"Fresh confirmation-side evidence is returning after rebuilt posture softened or reset for {label}, "
            "but stronger rebuilt posture still needs more fresh follow-through before it is re-earned."
        )
    if status == "pending-clearance-rebuild-reentry":
        return (
            f"Fresh clearance-side evidence is returning after rebuilt posture softened or reset for {label}, "
            "but stronger rebuilt posture still needs more fresh follow-through before it is re-earned."
        )
    if status == "reentered-confirmation-rebuild":
        return (
            f"Fresh confirmation-side follow-through for {label} has re-earned stronger rebuilt confirmation posture."
        )
    if status == "reentered-clearance-rebuild":
        return (
            f"Fresh clearance-side pressure for {label} has re-earned stronger rebuilt clearance posture."
        )
    if status == "blocked":
        return primary_target.get(
            "closure_forecast_reset_reentry_rebuild_reentry_reason",
    ...
    if recovering_confirmation_hotspots:
        hotspot = recovering_confirmation_hotspots[0]
        return (
            f"Confirmation-side rebuilt re-entry is closest around {hotspot.get('label', 'recent hotspots')}, "
            "but it still needs one more layer of fresh confirmation follow-through."
        )
    if recovering_clearance_hotspots:
        hotspot = recovering_clearance_hotspots[0]
        return (
            f"Clearance-side rebuilt re-entry is closest around {hotspot.get('label', 'recent hotspots')}, "
            "but it still needs one more layer of fresh clearance follow-through."
        )
    return "No rebuilt re-entry control is changing the current restored closure-forecast posture right now."
        ...
        events.append(
            {
                ...
                "closure_forecast_reset_reentry_rebuild_refresh_recovery_status": current_primary_target.get(
                    "closure_forecast_reset_reentry_rebuild_refresh_recovery_status",
                    "none",
                ),
                "closure_forecast_reset_reentry_rebuild_reentry_status": current_primary_target.get(
                    "closure_forecast_reset_reentry_rebuild_reentry_status",
                    "none",
                ),
            }
        ...
        events.append(
            {
                ...
                "closure_forecast_reset_reentry_rebuild_refresh_recovery_status": summary.get(
                    "primary_target_closure_forecast_reset_reentry_rebuild_refresh_recovery_status",
                    primary_target.get(
                        "closure_forecast_reset_reentry_rebuild_refresh_recovery_status",
                        "none",
                ...
                "closure_forecast_reset_reentry_rebuild_reentry_status": summary.get(
                    "primary_target_closure_forecast_reset_reentry_rebuild_reentry_status",
                    primary_target.get(
                        "closure_forecast_reset_reentry_rebuild_reentry_status",
                        "none",
