...
def _operator_transition_closure_values(data: dict) -> tuple[str, str, str, str, str, str, str]:
    ...
    reset_reentry_rebuild_persistence = (
        summary.get("primary_target_closure_forecast_reset_reentry_rebuild_persistence_status", "")
        or "none"
    ).replace("-", " ").title()
    reset_reentry_rebuild_churn = (
        summary.get("primary_target_closure_forecast_reset_reentry_rebuild_churn_status", "") or "none"
    ).replace("-", " ").title()
    closure_summary = (
        summary.get("closure_forecast_reset_reentry_rebuild_persistence_summary")
    ...
    if excel_mode == "standard":
        operator_rows.extend(
            [
                ...
                ("Reset Re-entry Rebuild Persistence", reset_reentry_rebuild_persistence),
                ("Reset Re-entry Rebuild Churn", reset_reentry_rebuild_churn),
...
def _build_executive_summary(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        narrative_rows.insert(34, ("Reset Re-entry Rebuild Persistence", reset_reentry_rebuild_persistence))
        narrative_rows.insert(35, ("Reset Re-entry Rebuild Churn", reset_reentry_rebuild_churn))
    ...
    if operator_summary:
        ...
        if excel_mode == "standard":
            ...
            ws.cell(row=63, column=4, value="Reset Re-entry Rebuild Persistence").font = SUBHEADER_FONT
            ws.cell(row=63, column=5, value=reset_reentry_rebuild_persistence)
            ws.cell(row=64, column=4, value="Reset Re-entry Rebuild Churn").font = SUBHEADER_FONT
            ws.cell(row=64, column=5, value=reset_reentry_rebuild_churn)
...
def _build_print_pack(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        ws["A48"] = "Reset Re-entry Rebuild Persistence"
        ws["B48"] = reset_reentry_rebuild_persistence
        ws["A49"] = "Reset Re-entry Rebuild Churn"
        ws["B49"] = reset_reentry_rebuild_churn
