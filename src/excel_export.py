...
def _operator_transition_closure_values(data: dict) -> tuple[str, str, str, str, str, str, str]:
    ...
    reset_reentry_rebuild_freshness = (
        summary.get("primary_target_closure_forecast_reset_reentry_rebuild_freshness_status", "")
        or "insufficient-data"
    ).replace("-", " ").title()
    reset_reentry_rebuild_reset = (
        summary.get("primary_target_closure_forecast_reset_reentry_rebuild_reset_status", "") or "none"
    ).replace("-", " ").title()
    closure_summary = (
        summary.get("closure_forecast_reset_reentry_rebuild_freshness_summary")
    ...
    if excel_mode == "standard":
        operator_rows.extend(
            [
                ...
                ("Reset Re-entry Rebuild Freshness", reset_reentry_rebuild_freshness),
                ("Reset Re-entry Rebuild Reset", reset_reentry_rebuild_reset),
...
def _build_executive_summary(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        narrative_rows.insert(34, ("Reset Re-entry Rebuild Freshness", reset_reentry_rebuild_freshness))
        narrative_rows.insert(35, ("Reset Re-entry Rebuild Reset", reset_reentry_rebuild_reset))
    ...
    if operator_summary:
        ...
        if excel_mode == "standard":
            ...
            ws.cell(row=63, column=4, value="Reset Re-entry Rebuild Freshness").font = SUBHEADER_FONT
            ws.cell(row=63, column=5, value=reset_reentry_rebuild_freshness)
            ws.cell(row=64, column=4, value="Reset Re-entry Rebuild Reset").font = SUBHEADER_FONT
            ws.cell(row=64, column=5, value=reset_reentry_rebuild_reset)
...
def _build_print_pack(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        ws["A48"] = "Reset Re-entry Rebuild Freshness"
        ws["B48"] = reset_reentry_rebuild_freshness
        ws["A49"] = "Reset Re-entry Rebuild Reset"
        ws["B49"] = reset_reentry_rebuild_reset
