...
def _operator_transition_closure_values(data: dict) -> tuple[str, str, str, str, str, str, str]:
    ...
    reset_reentry_refresh = (
        summary.get("primary_target_closure_forecast_reset_reentry_refresh_recovery_status", "")
        or "none"
    ).replace("-", " ").title()
    reset_reentry_rebuild = (
        summary.get("primary_target_closure_forecast_reset_reentry_rebuild_status", "") or "none"
    ).replace("-", " ").title()
    closure_summary = (
        summary.get("closure_forecast_reset_reentry_rebuild_summary")
    ...
    if excel_mode == "standard":
        operator_rows.extend(
            [
                ...
                ("Reset Re-entry Refresh", reset_reentry_refresh),
                ("Reset Re-entry Rebuild", reset_reentry_rebuild),
...
def _build_executive_summary(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        narrative_rows.insert(34, ("Reset Re-entry Refresh", reset_reentry_refresh))
        narrative_rows.insert(35, ("Reset Re-entry Rebuild", reset_reentry_rebuild))
    ...
    if operator_summary:
        ...
        if excel_mode == "standard":
            ...
            ws.cell(row=63, column=4, value="Reset Re-entry Refresh").font = SUBHEADER_FONT
            ws.cell(row=63, column=5, value=reset_reentry_refresh)
            ws.cell(row=64, column=4, value="Reset Re-entry Rebuild").font = SUBHEADER_FONT
            ws.cell(row=64, column=5, value=reset_reentry_rebuild)
...
def _build_print_pack(
    ...
    *,
    ...
    if excel_mode == "standard":
        ...
        ws["A48"] = "Reset Re-entry Refresh"
        ws["B48"] = reset_reentry_refresh
        ws["A49"] = "Reset Re-entry Rebuild"
        ws["B49"] = reset_reentry_rebuild
