...
SKIP_DIRS = frozenset({
    ...
    ".claude", ".serena", ".pytest_cache", ".mypy_cache", ".ruff_cache",
    "output", "htmlcov",
})
...
class SecurityAnalyzer(BaseAnalyzer):
    ...
    def analyze(
        ...
        supported_ecosystems = _detect_dependabot_supported_ecosystems(repo_path)
        details["dependabot_supported"] = bool(supported_ecosystems)
        details["dependabot_supported_ecosystems"] = supported_ecosystems

        if not details["has_security_md"]:
            score -= 0.1
            findings.append("No SECURITY.md")
        if not details["has_dependabot"] and details["dependabot_supported"]:
            score -= 0.1
            findings.append("No Dependabot config")
        elif not details["has_dependabot"]:
            findings.append("No supported Dependabot ecosystem detected")
...
def _detect_dependabot_supported_ecosystems(repo_path: Path) -> list[str]:
    """Return Dependabot ecosystems backed by manifests or workflows in this repo."""
    ...
    for path in repo_path.rglob("*"):
        ...
        if any(part in SKIP_DIRS for part in path.parts):
            continue
        for ecosystem, filenames in DEPENDABOT_ECOSYSTEM_FILES.items():
