...
_CONTEXTUAL_SCORECARD_HIGH_RULES = {
    "CodeReviewID",
    "MaintainedID",
}
...
class GitHubClient:
    ...
    def get_code_scanning_alert_count(self, owner: str, repo: str) -> dict:
        """Fetch open code scanning alert counts when accessible."""
        try:
            data = self._fetch_json(
            ...
            alerts = data if isinstance(data, list) else []
            severity_counts = {"critical": 0, "high": 0, "warning": 0, "note": 0}
            high_rule_counts: dict[str, int] = {}
            contextual_high = 0
            for alert in alerts:
                ...
                elif raw in {"high", "error"}:
                    severity_counts["high"] += 1
                    if rule_id:
                        high_rule_counts[rule_id] = high_rule_counts.get(rule_id, 0) + 1
                    if rule_id in _CONTEXTUAL_SCORECARD_HIGH_RULES:
                        contextual_high += 1
                elif raw in {"medium", "low", "warning"}:
            ...
            return {
                ...
                "open_alerts": len(alerts),
                **severity_counts,
                "high_rule_counts": high_rule_counts,
                "contextual_high": contextual_high,
                "actionable_high": max(0, severity_counts["high"] - contextual_high),
            }
        except requests.HTTPError as exc:
            status = self._http_error_status(exc)
            self._log_security_endpoint_error(
