...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS campaign_tuning (
            run_id TEXT NOT NULL,
            campaign_type TEXT NOT NULL,
            tuning_status TEXT NOT NULL,
            recommendation_bias TEXT NOT NULL,
            judged_count INTEGER NOT NULL,
            holding_clean_rate REAL NOT NULL,
            drift_return_rate REAL NOT NULL,
            reopen_rate REAL NOT NULL,
            rollback_watch_rate REAL NOT NULL,
            pressure_reduction_rate REAL NOT NULL,
            details_json TEXT NOT NULL DEFAULT '{}',
            PRIMARY KEY (run_id, campaign_type)
        );
        ...
        CREATE TABLE IF NOT EXISTS intervention_ledger (
            run_id TEXT NOT NULL,
            repo TEXT NOT NULL,
            historical_intelligence_status TEXT NOT NULL,
            pressure_trend TEXT NOT NULL,
            hotspot_persistence TEXT NOT NULL,
            scorecard_trend TEXT NOT NULL,
            campaign_follow_through TEXT NOT NULL,
            recent_intervention_count INTEGER NOT NULL,
            details_json TEXT NOT NULL DEFAULT '{}',
        ...
        CREATE TABLE IF NOT EXISTS repo_implementation_hotspots (
            ...
            details_json TEXT NOT NULL DEFAULT '{}',
            PRIMARY KEY (run_id, repo, path, category)
        );
...
def _insert_run(conn: sqlite3.Connection, report: AuditReport, report_path: Path | None) -> None:
    ...
    for audit in report.audits:
        ...
        for hotspot in audit.implementation_hotspots:
            conn.execute(
                """
                INSERT OR REPLACE INTO repo_implementation_hotspots (
                    run_id, repo, path, category, pressure_score, suggestion_type, details_json
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    run_id,
                    ...
                    float(hotspot.get("pressure_score", 0.0) or 0.0),
    ...
    for tuning in report.action_sync_tuning:
        conn.execute(
            """
            INSERT OR REPLACE INTO campaign_tuning (
                run_id, campaign_type, tuning_status, recommendation_bias, judged_count,
                holding_clean_rate, drift_return_rate, reopen_rate, rollback_watch_rate,
                pressure_reduction_rate, details_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                run_id,
                tuning.get("campaign_type", ""),
                tuning.get("tuning_status", "insufficient-evidence"),
                tuning.get("recommendation_bias", "neutral"),
                int(tuning.get("judged_count", 0) or 0),
                float(tuning.get("holding_clean_rate", 0.0) or 0.0),
                float(tuning.get("drift_return_rate", 0.0) or 0.0),
                float(tuning.get("reopen_rate", 0.0) or 0.0),
                float(tuning.get("rollback_watch_rate", 0.0) or 0.0),
                float(tuning.get("pressure_reduction_rate", 0.0) or 0.0),
                json.dumps(tuning),
            ),
        )

    for row in report.historical_portfolio_intelligence:
        conn.execute(
            """
            INSERT OR REPLACE INTO intervention_ledger (
                run_id, repo, historical_intelligence_status, pressure_trend, hotspot_persistence,
                scorecard_trend, campaign_follow_through, recent_intervention_count, details_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                run_id,
                row.get("repo", ""),
                row.get("historical_intelligence_status", "insufficient-evidence"),
                row.get("pressure_trend", "insufficient-evidence"),
                row.get("hotspot_persistence", "insufficient-evidence"),
                row.get("scorecard_trend", "insufficient-evidence"),
                row.get("campaign_follow_through", "insufficient-evidence"),
                int(row.get("recent_intervention_count", 0) or 0),
                json.dumps(row),
            ),
        )
...
def _coerce_action_sync_snapshot_field(operator_summary: dict, key: str, default):
    value = operator_summary.get(key, default)
    if value is None:
        return default
    return value
...
def load_latest_audit_runs(output_dir: Path, username: str, limit: int = 20) -> list[dict]:
    ...
    for row in rows:
        ...
        if not decision_quality_summary:
            from github_repo_auditor import operator_decision_quality as _operator_decision_quality
        ...
        results.append(
            {
                ...
                "campaign_readiness_summary": _coerce_action_sync_snapshot_field(
                    operator_summary, "campaign_readiness_summary", {}
                ),
                "action_sync_summary": _coerce_action_sync_snapshot_field(
                    operator_summary, "action_sync_summary", {}
                ),
                "next_action_sync_step": _coerce_action_sync_snapshot_field(
                    operator_summary, "next_action_sync_step", ""
                ),
                "action_sync_packets": _coerce_action_sync_snapshot_field(
                    operator_summary, "action_sync_packets", []
                ),
                "apply_readiness_summary": _coerce_action_sync_snapshot_field(
                    operator_summary, "apply_readiness_summary", {}
                ),
                "next_apply_candidate": _coerce_action_sync_snapshot_field(
                    operator_summary, "next_apply_candidate", {}
                ),
                "action_sync_outcomes": _coerce_action_sync_snapshot_field(
                    operator_summary, "action_sync_outcomes", []
                ),
                "next_monitoring_step": _coerce_action_sync_snapshot_field(
                    operator_summary, "next_monitoring_step", {}
                ),
                "action_sync_tuning": _coerce_action_sync_snapshot_field(
                    operator_summary, "action_sync_tuning", []
                ),
                ...
                "next_historical_focus": _coerce_action_sync_snapshot_field(
                    operator_summary, "next_historical_focus", {}
                ),
            }
        )
    return results
...
def load_recent_campaign_history(output_dir: Path, username: str, limit: int = 200) -> list[dict]:
    conn = _connect(output_dir)
    if conn is None:
        return []
    try:
        rows = conn.execute(
            """
            SELECT audit_runs.generated_at,
                   campaign_history.action_id,
                   campaign_history.repo_id,
                   campaign_history.campaign_type,
                   campaign_history.lifecycle_state,
                   campaign_history.reconciliation_outcome,
                   campaign_history.closed_at,
                   campaign_history.closed_reason,
                   campaign_history.reopened_at,
                   campaign_history.details_json
            FROM campaign_history
            JOIN audit_runs ON audit_runs.run_id = campaign_history.run_id
            WHERE audit_runs.username = ?
            ORDER BY audit_runs.generated_at DESC, campaign_history.action_id
            LIMIT ?
            """,
            (username, limit),
        ).fetchall()
        ...
        history.append(
            {
                ...
                "action_id": row["action_id"],
                "repo_id": row["repo_id"],
                "repo": details.get("repo") or row["repo_id"],
                "campaign_type": row["campaign_type"],
                "lifecycle_state": row["lifecycle_state"],
                "reconciliation_outcome": row["reconciliation_outcome"],
                "closed_at": row["closed_at"],
...
def load_campaign_tuning(output_dir: Path, username: str, limit: int = 60) -> list[dict]:
    conn = _connect(output_dir)
    if conn is None:
        return []
    try:
        rows = conn.execute(
            """
            SELECT campaign_tuning.run_id,
                   audit_runs.generated_at,
                   campaign_tuning.campaign_type,
                   campaign_tuning.tuning_status,
                   campaign_tuning.recommendation_bias,
                   campaign_tuning.judged_count,
                   campaign_tuning.holding_clean_rate,
                   campaign_tuning.drift_return_rate,
                   campaign_tuning.reopen_rate,
                   campaign_tuning.rollback_watch_rate,
                   campaign_tuning.pressure_reduction_rate,
                   campaign_tuning.details_json
            FROM campaign_tuning
            JOIN audit_runs ON audit_runs.run_id = campaign_tuning.run_id
            WHERE audit_runs.username = ?
            ORDER BY audit_runs.generated_at DESC
            LIMIT ?
            """,
...
def load_recent_repo_scorecards(output_dir: Path, username: str, limit: int = 200) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            """
            SELECT repo_scorecards.run_id,
                   audit_runs.generated_at,
                   repos.name AS repo,
                   repo_scorecards.program,
                   repo_scorecards.program_label,
                   repo_scorecards.target_maturity,
                   repo_scorecards.maturity_level,
                   repo_scorecards.score,
                   repo_scorecards.status,
                   repo_scorecards.summary
            FROM repo_scorecards
            JOIN audit_runs ON audit_runs.run_id = repo_scorecards.run_id
            JOIN repos ON repos.repo_id = repo_scorecards.repo_id
            WHERE audit_runs.username = ?
            ORDER BY audit_runs.generated_at DESC
            LIMIT ?
            """,
...
def load_recent_implementation_hotspots(
    ...
    try:
        rows = conn.execute(
            """
            SELECT repo_implementation_hotspots.run_id,
                   audit_runs.generated_at,
                   repo_implementation_hotspots.repo,
                   repo_implementation_hotspots.path,
                   repo_implementation_hotspots.category,
                   repo_implementation_hotspots.pressure_score,
                   repo_implementation_hotspots.suggestion_type,
                   repo_implementation_hotspots.details_json
            FROM repo_implementation_hotspots
            JOIN audit_runs ON audit_runs.run_id = repo_implementation_hotspots.run_id
            WHERE audit_runs.username = ?
            ORDER BY audit_runs.generated_at DESC, repo_implementation_hotspots.repo, repo_implementation_hotspots.path
            LIMIT ?
            """,
            (username, limit),
        ).fetchall()
...
def load_intervention_ledger(output_dir: Path, username: str, limit: int = 200) -> list[dict]:
    conn = _connect(output_dir)
    if conn is None:
        return []
    try:
        rows = conn.execute(
            """
            SELECT intervention_ledger.run_id,
                   audit_runs.generated_at,
                   intervention_ledger.repo,
                   intervention_ledger.historical_intelligence_status,
                   intervention_ledger.pressure_trend,
                   intervention_ledger.hotspot_persistence,
                   intervention_ledger.scorecard_trend,
                   intervention_ledger.campaign_follow_through,
                   intervention_ledger.recent_intervention_count,
                   intervention_ledger.details_json
            FROM intervention_ledger
            JOIN audit_runs ON audit_runs.run_id = intervention_ledger.run_id
            WHERE audit_runs.username = ?
            ORDER BY audit_runs.generated_at DESC, intervention_ledger.repo
            LIMIT ?
            """,
            (username, limit),
        ).fetchall()
