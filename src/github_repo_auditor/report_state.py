...
from github_repo_auditor.registry_parser import RegistryReconciliation
...
def audit_from_dict(data: dict) -> RepoAudit:
    ...
    return RepoAudit(
        ...
        degraded_dimensions=data.get("degraded_dimensions", []),
...
def report_from_dict(data: dict) -> AuditReport:
    ...
    return AuditReport(
        ...
        audits=[audit_from_dict(audit) for audit in data.get("audits", [])],
        ...
        portfolio_baseline_size=data.get(
            "portfolio_baseline_size", len(data.get("audits", []))
        ),
        ...
        historical_portfolio_intelligence=data.get(
