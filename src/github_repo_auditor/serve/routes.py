...
from github_repo_auditor.serve.runner import SAFE_FLAG_NAMES, get_session, spawn_run, validate_flags
...
async def initiatives_suggestions(
    ...
    from github_repo_auditor.suggest_initiatives import default_deadline_for_effort, generate_suggestions
...
async def dismiss_suggestion_route(
    ...
    from github_repo_auditor.suggest_initiatives import dismiss_suggestion_record, dismissed_path
...
async def initiatives_dismissal_history(request: Request) -> HTMLResponse:
    ...
    from github_repo_auditor.suggest_initiatives import dismissed_path, load_dismissal_events
...
async def initiative_gap(request: Request, repo_name: str, target: int = 0) -> HTMLResponse:
    ...
    if target not in (1, 2, 3, 4):
        ...
        try:
            from github_repo_auditor.initiatives import initiatives_path, load_initiatives
