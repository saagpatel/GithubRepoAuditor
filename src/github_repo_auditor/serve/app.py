...
def create_app(output_dir: Path | None = None) -> "FastAPI":  # noqa: F821
    ...
    from github_repo_auditor.serve.api import router as api_router
    from github_repo_auditor.serve.hosting import (
        build_kv_store,
        build_rate_limiter,
        build_report_cache,
    )
    from github_repo_auditor.serve.routes import router
    from github_repo_auditor.serve.waitlist import build_waitlist_store
