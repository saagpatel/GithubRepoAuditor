...
from github_repo_auditor.portfolio_pathing import VALID_OPERATING_PATHS
...
def catalog_entry_for_repo(
    ...
    project_path = _safe_text(metadata.get("path"))
    normalized_project_path = _normalize_key(project_path)
    ...
    matched = repos.get(normalized_project_path)
    if matched:
        return {
            **matched,
            "repo": repo_name,
