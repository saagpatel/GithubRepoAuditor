...
def run_portfolio_truth_mode(args: Any) -> None:
    ...
    if getattr(args, "portfolio_truth_include_degraded_dimensions", False):
        degraded_dimensions_by_name = load_degraded_dimensions_by_name(
            output_dir=output_dir,
            username=args.username,
        )
    ...
    if getattr(args, "portfolio_truth_include_security", False):
        ...
        if loaded_security is not None:
            ...
            security_coverage_metadata = {
                ...
                "cohort_repository_count": len(loaded_security.cohort_repositories),
    ...
    try:
        result = publish_portfolio_truth(
            ...
            include_notion=True,
            allow_empty_notion=getattr(
                args, "portfolio_truth_allow_empty_notion", False
            ),
    ...
    print_info(
        f"Portfolio audit compatibility output: {result.portfolio_report_output}"
    )
    print_info(
...
def run_portfolio_context_recovery_mode(args: Any) -> None:
    ...
    plan_json, plan_markdown = write_context_recovery_plan_artifacts(
    ...
    print_info(
        f"Portfolio audit compatibility output: {truth_result.portfolio_report_output}"
    )
