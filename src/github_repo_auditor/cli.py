...
from github_repo_auditor.cli_mode_validation import validate_cli_mode_args
from github_repo_auditor.cli_output import print_info
from github_repo_auditor.app.run_audit import (
    _run_main_audit_cycle,
    _run_watch_mode,
)
from github_repo_auditor.app.auto_apply import run_auto_apply_approved_mode
from github_repo_auditor.app.automation_proposals import run_automation_proposals_mode
from github_repo_auditor.app.initiatives import (
    run_close_initiative_mode,
    run_list_initiatives_mode,
    run_set_initiative_mode,
)
from github_repo_auditor.app.initiative_suggestions import (
...
from github_repo_auditor.app.campaign_workflow import (
    _run_campaign_from_ledger_mode,
    _run_draft_readmes_mode,
    _run_plan_campaign_mode,
)
from github_repo_auditor.app.portfolio_analysis import (
    _run_context_triage_mode,
    _run_tier_gaps_export_mode,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--portfolio-truth-include-degraded-dimensions",
        action="store_true",
        help=(
            "Overlay derived.degraded_dimensions on each project from the latest "
            "output/audit-report-<username>-*.json warehouse file, naming the analyzer "
            "dimensions that crashed and were excluded from the score (requires a prior audit run)"
        ),
    )
    parser.add_argument(
        "--portfolio-truth-include-security",
...
def build_pr_head_evidence_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "snapshot", help="Path to a local PRHeadEvidenceV1 JSON snapshot"
    )
    return parser
...
def _run_control_center_mode(args, parser) -> None:
    from github_repo_auditor.app.control_center import run_control_center_mode

    run_control_center_mode(args, parser)


def _run_approval_center_mode(args, parser) -> None:
    from github_repo_auditor.app.approval_center import run_approval_center_mode

    run_approval_center_mode(args, parser)


def _run_approval_capture_mode(args, parser) -> None:
    from github_repo_auditor.app.approval_center import run_approval_capture_mode

    run_approval_capture_mode(args, parser)


def _run_acknowledgment_capture_mode(args, parser) -> None:
    from github_repo_auditor.app.acknowledgments import run_acknowledgment_capture_mode

    run_acknowledgment_capture_mode(args, parser)


def _run_doctor_mode(args, config_inspection) -> None:
    from github_repo_auditor.app.doctor import run_doctor_mode
...
def _run_portfolio_context_recovery_mode(args) -> None:
    from github_repo_auditor.app.portfolio_truth import run_portfolio_context_recovery_mode
...
def _run_serve_mode(args: object) -> None:
    from github_repo_auditor.app.serve_mode import run_serve_mode
...
def _run_security_burndown_mode(args) -> None:
    from github_repo_auditor.app.security_modes import run_security_burndown_mode

    run_security_burndown_mode(args)


def _run_security_gate_mode(args) -> None:
    from github_repo_auditor.app.security_modes import run_security_gate_mode
...
def main() -> None:
    ...
    from github_repo_auditor.approval_ledger import default_approval_reviewer
    ...
    from github_repo_auditor.config import inspect_config, merge_config_with_args
    ...
    if getattr(args, "reset_prefs", False):
        from github_repo_auditor.operator_prefs import prefs_path, reset_prefs
