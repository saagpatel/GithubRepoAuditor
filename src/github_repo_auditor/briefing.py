...
from github_repo_auditor.narrative import NarrativeProvider, _resolve_provider
...
def _build_health_delta(
    ...
    try:
        from github_repo_auditor.history import load_repo_score_history
...
def _build_suggested_initiatives(
    ...
    """
    try:
        from github_repo_auditor.suggest_initiatives import generate_suggestions
    except ImportError:
...
def build_briefing(
    ...
    *,
    ...
    Pass *semantic_index* (a :class:`~github_repo_auditor.semantic_index.SemanticIndex` instance)
    ...
    Pass *cost_tracker* (a :class:`~github_repo_auditor.llm_cost.CostTracker` instance) to record
...
def render_markdown(briefing: Briefing) -> str:
    ...
    if briefing.initiatives:
        ...
        today = _date.today()
        for ini in briefing.initiatives:
            from github_repo_auditor.maturity_tiers import tier_name as _tier_name
    ...
    if briefing.suggested_initiatives:
        from github_repo_auditor.maturity_tiers import tier_name as _tier_name2
...
def generate_briefing(
    ...
    *,
    ...
    Pass *semantic_index* (a :class:`~github_repo_auditor.semantic_index.SemanticIndex` instance) to
    ...
    if result is not None:
        ...
        if model is None:
            from github_repo_auditor.narrative import DEFAULT_ANTHROPIC_MODEL, DEFAULT_GITHUB_MODELS_MODEL
    ...
    from github_repo_auditor.operator_prefs import load_prefs, prefs_path
