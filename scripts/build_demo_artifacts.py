...
# Portfolio Command Center discovers these artifacts by filename prefix, so a
# superseded file from an earlier generation would still be picked up.
PCC_ARTIFACT_PREFIXES = (
    "portfolio-truth-",
    "weekly-command-center-",
    "security-burndown-",
)
WEEKLY_DIGEST_FILENAME = "weekly-command-center-demo.json"
SECURITY_BURNDOWN_FILENAME = "security-burndown-demo.json"
PROPOSALS_FILENAME = "pending-proposals.json"
...
def main() -> None:
    ...
    export_excel(
        report_path,
        OUTPUT_DIR / "demo-workbook.xlsx",
        diff_data=diff_data,
        excel_mode="standard",
    )
    _write_demo_command_center_artifacts()
...
def _write_demo_command_center_artifacts() -> None:
    """Regenerate every Portfolio Command Center artifact from the synthetic fixture.

    The demo truth is intentionally independent of the sample audit report: the
    ...
    for existing in OUTPUT_DIR.glob("*.json"):
        if existing.name.startswith(PCC_ARTIFACT_PREFIXES):
            existing.unlink()

    generated_at = fixture_generated_at()
    snapshot = build_snapshot(generated_at)
    _write_json(OUTPUT_DIR / TRUTH_LATEST_FILENAME, snapshot)
    for name, history in history_snapshots(generated_at):
        _write_json(OUTPUT_DIR / name, history)
    _write_json(OUTPUT_DIR / WEEKLY_DIGEST_FILENAME, build_weekly_digest(snapshot))
    _write_json(
        OUTPUT_DIR / SECURITY_BURNDOWN_FILENAME, build_security_burndown(snapshot)
    )
    _write_json(OUTPUT_DIR / PROPOSALS_FILENAME, build_proposals(generated_at))
...
def _write_demo_warehouse(report_data: dict[str, Any], report_path) -> None:
    ...
    for item in report_data.get("audits", []):
        ...
        full_name = (
            ...
            or f"{report_data.get('username', 'sample-user')}/{repo_name}"
        )
        analyzer_results = [
            AnalyzerResult(
    ...
    report = AuditReport(
        ...
        security_governance_preview=list(
            report_data.get("security_governance_preview", [])
        ),
