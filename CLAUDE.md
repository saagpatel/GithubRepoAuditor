...
## Current State
Workbook-first portfolio operator with shared JSON, Markdown, Excel, HTML, review-pack, control-center, scheduled-handoff, approval-center, and portfolio-truth surfaces. Do not trust hardcoded test or test-file counts in docs; rerun `python3 -m pytest -q -p no:cacheprovider` and inspect `tests/` before reporting release status. Use `docs/plans/` for current phase status, deferred work, and closeout workflow.
...
Complete manual workbook signoff if the 2026-04-24 rehearsal should become a release record. Phase 123 has its first three catalog opt-ins (`mcpforge`, `TradeOffAtlas`, `TideEngine`), and campaign packets plus approval records carry an `automation_subset` so operators can verify which actions belong to opted-in repos. The local `promotion-push` approval has been captured for the single automation-eligible `TideEngine` action, but live apply remains blocked: decision quality is `use-with-review`, so `--auto-apply-approved --dry-run` reports 0 full trust-bar repos and applies nothing. The latest control-center text now shows trust recovery progress at 2/3 stable run(s), with 1 more stable confirming run needed. Keep this dry-run-only and only revisit auto-apply after `decision_quality_status` returns to `trusted`. See docs/plans/2026-04-24-phase-123-readiness-prep.md, docs/plans/2026-04-24-post-merge-current-state.md, and docs/plans/2026-04-15-arc-d-closeout.md for the current gates and Arc E/F/G sequencing.
