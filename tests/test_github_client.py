...
class TestGitHubClientHardening:
    ...
    def test_security_endpoints_return_counts_when_available(self, monkeypatch):
        ...
        code_scanning = client.get_code_scanning_alert_count("o", "r")
        assert code_scanning["open_alerts"] == 5
        assert code_scanning["critical"] == 1
        assert code_scanning["high"] == 2
        assert code_scanning["contextual_high"] == 1
        assert code_scanning["actionable_high"] == 1
        assert code_scanning["high_rule_counts"] == {
            "CodeReviewID": 1,
            "ConcreteRuleID": 1,
        }
        assert code_scanning["warning"] == 1
        assert code_scanning["note"] == 1
