...
class TestCodeScanningBucketing:
    ...
    def test_code_scanning_prefers_security_severity_level(self) -> None:
        cs_alerts = [
            {"rule": {"severity": "error", "security_severity_level": "high"}},
            {"rule": {"severity": "error", "security_severity_level": "medium"}},
            {"rule": {"severity": "error", "security_severity_level": "low"}},
        ]
        session = _mock_session_get({
            "dependabot": [],
            "code-scanning": cs_alerts,
        ...
        result = fetch_ghas_alerts(
            [_make_audit("scorecard-repo")],
            token="tok",
            session=session,
        )
        cs = result["scorecard-repo"]["code_scanning"]
        assert cs["high"] == 1
        assert cs["warning"] == 2
