...
class TestHelpFlagCounts:
    ...
    def test_report_help_flag_count(self):
        ...
        count = _count_flags_in_help(text)
        assert count <= 47, (
            f"audit report --help shows {count} non-global flags (limit 47, raised for the "
            "bounded cohort-transition controls --portfolio-truth-security-max-cohort-size "
            "and --portfolio-truth-require-cohort-transition; the legacy "
            "--portfolio-truth-security-cohort-count alias is deliberately retained for one "
            "release so a stale deployed wrapper cannot hard-error on an unknown argument).\n"
            f"Flags found: {sorted(set(re.findall(r'  (--[a-z][a-z0-9-]*)', text)))}"
        )
