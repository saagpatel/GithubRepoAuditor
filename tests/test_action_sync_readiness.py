...
def test_active_campaign_with_missing_rollback_stays_blocked():
    report = _report_data(
        campaign_summary={
            "campaign_type": "security-review",
            "label": "Security Review",
    ...
    bundle = build_action_sync_readiness_bundle(report, queue=[])
    security = _campaign(bundle, "security-review")

    assert security["readiness_stage"] == "blocked"
    assert "Rollback coverage is missing" in security["reason"]
    assert bundle["top_apply_ready_campaigns"] == []
    assert bundle["next_action_sync_step"].startswith("Unblock Security Review")
    assert "blocked right now" in bundle["action_sync_summary"]["summary"]
