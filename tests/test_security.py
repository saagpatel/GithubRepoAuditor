...
class TestSecretScanning:
    ...
    def test_ignores_runtime_generated_shell_values(self, tmp_path):
        (tmp_path / "release.sh").write_text(
            ...
            'TWINE_USERNAME=__token__ TWINE_PASSWORD="$TWINE_API_TOKEN" twine upload dist/*\n'
        )
        found = _scan_secrets(tmp_path)
        assert found == []
    ...
    def test_skips_generated_and_agent_worktree_dirs(self, tmp_path):
        cache = tmp_path / "output" / ".cache"
        cache.mkdir(parents=True)
        _write_fake_secret(cache, "repo.json", "generic_secret")
        agent = tmp_path / ".claude" / "worktrees" / "agent-1"
        agent.mkdir(parents=True)
        _write_fake_secret(agent, "fixture.py", "private_key")
...
class TestSecurityAnalyzer:
    ...
    def test_missing_security_config_penalized(self, tmp_path):
        ...
        assert not result.details["dependabot_supported"]
        assert "No supported Dependabot ecosystem detected" in result.findings

    def test_missing_dependabot_only_penalized_for_supported_ecosystem(self, tmp_path):
        (tmp_path / "SECURITY.md").write_text("# Security Policy")
        (tmp_path / "project.godot").write_text("[application]\n")

        result = SecurityAnalyzer().analyze(tmp_path, _meta(language="GDScript", languages={"GDScript": 5000}))

        assert result.score == 1.0
        assert not result.details["dependabot_supported"]
        assert "No Dependabot config" not in result.findings

    def test_detects_dependabot_supported_manifest(self, tmp_path):
        (tmp_path / "SECURITY.md").write_text("# Security Policy")
        (tmp_path / "package.json").write_text('{"dependencies": {}}')

        result = SecurityAnalyzer().analyze(tmp_path, _meta(language="TypeScript", languages={"TypeScript": 5000}))

        assert result.score < 1.0
        assert result.details["dependabot_supported"]
        assert result.details["dependabot_supported_ecosystems"] == ["npm"]
        assert "No Dependabot config" in result.findings

    def test_detects_dependabot_supported_github_actions(self, tmp_path):
        (tmp_path / "SECURITY.md").write_text("# Security Policy")
        ...
        assert result.details["dependabot_supported"]
        assert result.details["dependabot_supported_ecosystems"] == ["github-actions"]
