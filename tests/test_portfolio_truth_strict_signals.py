...
Covers has_tests, has_ci, has_license, readme_char_count, and opt-in release_count.
"""
...
def test_has_license_with_root_license_file(tmp_path: Path) -> None:
    """Project with a LICENSE file at the root → has_license == True."""
    proj = _make_project(tmp_path)
    (proj / "LICENSE").write_text("MIT")
    assert _derive_has_license(proj, has_git=True) is True


def test_has_license_with_copying_file(tmp_path: Path) -> None:
    """Project with a COPYING file at the root → has_license == True."""
    proj = _make_project(tmp_path)
    (proj / "COPYING").write_text("GPL")
    assert _derive_has_license(proj, has_git=True) is True


def test_has_license_false_when_no_git(tmp_path: Path) -> None:
    """Project where has_git == False → has_license == False."""
    (tmp_path / "LICENSE").write_text("MIT")
    assert _derive_has_license(tmp_path, has_git=False) is False


def test_has_license_false_when_path_nonexistent(tmp_path: Path) -> None:
    """Project where path doesn't exist → has_license == False without crashing."""
    missing = tmp_path / "nonexistent_project"
    assert _derive_has_license(missing, has_git=True) is False
