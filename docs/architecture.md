...
The operator model now intentionally has two layers. `src/operator_control_center.py` remains the raw source of truth and keeps the detailed follow-through lifecycle families, including the deeper post-revalidation recovery states added in Phase 78. `src/report_enrichment.py` then compresses that model into a primary-surface contract built around five operator focus buckets: `Act Now`, `Watch Closely`, `Improving`, `Fragile`, and `Revalidate`.

That split is a product choice, not duplication. Hidden workbook sheets, raw JSON, and machine-facing artifacts keep the full detailed lifecycle fields for parity and future planning. Primary workbook, HTML, Markdown, and review-pack surfaces use the compressed operator focus contract so the operator loop stays fast to read without losing the underlying evidence.
